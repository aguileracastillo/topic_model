...
```{r redux db and rename var, include=FALSE}
DGRLv17_5_zotero_redux <- DGRLv17_5_zotero %>% select(1:6, 9, 11)
DGRLv17_5_zotero_redux <- DGRLv17_5_zotero_redux %>% rename(id = `Key`, type = `Item Type`, year = `Publication Year`, author = `Author`, doc_title = `Title`, pub_title = `Publication Title`, abstract = `Abstract Note`)
```
...
```{r all docs in DGRLv17.5, include = FALSE}
...
In its raw and unprocessed form, the data set has a large proportion of missing values `r miss_all_db`% but not all columns are relevant to this exploration. Just 8 columns will be retained as the contents are considered relevant covariates for the analysis proposed in this exercise.

```{r visualization redux 8 var, echo=FALSE, include=TRUE}
vis_miss(DGRLv17_5_zotero_redux)

```

By visually exploring the contents of the data set, it is evident that a large amount of relevant information is missing. Important covariates for the analysis have a substantial proportion of missing information like year, publication title and abstract, our main unit of analysis for the topic model.
